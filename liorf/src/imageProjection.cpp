...
public:
    ...
    bool cachePointCloud(const sensor_msgs::PointCloud2ConstPtr& laserCloudMsg)
    {
        ...
        //timeScanEnd = imuQueue.back().header.stamp.toSec() - 0.20;          //LSB
    ...
    void projectPointCloud()
    {
        ...
        for (int i = 0; i < cloudSize; ++i)
        {
            ...
            if (has_ring)
                ...
                // Blog: https://blog.csdn.net/qq_42938987/article/details/108434290
                double yaw_angle = atan2(thisPoint.y, thisPoint.x) * 57.295779;
                if (is_first[rowIdn])
                {
                    yaw_fp[rowIdn]=yaw_angle;
                    is_first[rowIdn]=false;
                    laserCloudIn->points[i].time = 0.0;
                    yaw_last[rowIdn]=yaw_angle;
                    time_last[rowIdn]=laserCloudIn->points[i].time;
                    continue;
                }

                if (yaw_angle <= yaw_fp[rowIdn])
                {
                    laserCloudIn->points[i].time = (yaw_fp[rowIdn]-yaw_angle) / omega_l;
                }
                else
                {
                    laserCloudIn->points[i].time = (yaw_fp[rowIdn]-yaw_angle+360.0) / omega_l;
                }

                if (laserCloudIn->points[i].time < time_last[rowIdn])  
                    laserCloudIn->points[i].time+=360.0/omega_l;

                yaw_last[rowIdn] = yaw_angle;
                time_last[rowIdn]=laserCloudIn->points[i].time;

                thisPoint = deskewPoint(&thisPoint, laserCloudIn->points[i].time * 1e-03);
            }
