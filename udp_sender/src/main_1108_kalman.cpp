...
KalmanFilter kalman_filter_east(0.0, 0.1, 0.01, 1.0);  // Tweak the parameters
KalmanFilter kalman_filter_north(0.0, 0.1, 0.01, 1.0);  // Tweak the parameters
...
int main(int argc, char **argv) {
    ...
    int map_flag = 0;               // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!Should be change with subscribe data
    ...
    while (ros::ok()) {
        ...
        if(map_flag == 1){
            if(count < 30){
                ...
                udp_data.error_flag = 1;    // when slam data initializing, set error_flag to be 1
            }
        ...
        // Map matching fail        
        //else if(map_flag == 0 && norm(속도) < 0.5){
        else if(map_flag == 0){
            if(count < 30){
                init_heading = (data_liorf.heading*rad2deg + bias)*(3.141592/180.0);
                init_east = data_liorf.east;
                init_north = data_liorf.north;

                init_slam_x = data_liorf.utm_x;
                init_slam_y = data_liorf.utm_y;
