...
int main(int argc, char **argv) {
    ...
    while (ros::ok()) {
        ...
        if(map_flag_done == 1){
            ...
            init_heading = (108.9922)*(3.141592/180.0);
            init_east = 320942.1875;
            init_north = 4155025.5;
        ...
        // Map matching fail
        //else if (map_flag != 1 && liorf_disable < 1 && count > init_done_size){         
        else if (liorf_disable < 1 && count > init_done_size-1){         
...
/*         // Odom
        ...
        odom.child_frame_id = "odom_mapping"; // 추가: child_frame_id 설정

        //set the position
        odom.pose.pose.position.x = val.north;
        odom.pose.pose.position.y = val.east;
        //odom.pose.pose.position.z = (data.down - 37.18);
        odom.pose.pose.position.z = data_liorf.down;
        
        quat.setRPY(0.0, 0.0, -head); // roll, pitch, yaw를 Quaternion으로 변환
        odom.pose.pose.orientation.x = quat.x(); // Quaternion 값을 orientation으로 설정
        odom.pose.pose.orientation.y = quat.y();
        odom.pose.pose.orientation.z = quat.z();
        odom.pose.pose.orientation.w = quat.w();

        odom_pub.publish(odom); // odom publish

        // Create and publish transform between map_lio and odom
        geometry_msgs::TransformStamped transformStamped;
        transformStamped.header.stamp = current_time;
        transformStamped.header.frame_id = "map_lio";
        ...
        // Broadcast the transform between map_lio and odom
        odom_broadcaster.sendTransform(transformStamped);

        // Path
        // Pub Nav Path data
        path_msg.header.stamp = current_time;
        path_msg.header.frame_id = "odom"; // Set the frame ID appropriately
        odom.child_frame_id = "path_mapping"; // 추가: child_frame_id 설정
        
        // Fill in the poses in the path (assuming you have x, y, z positions)
        geometry_msgs::PoseStamped pose;
        pose.header.frame_id = "map_lio"; // Set the frame ID for each pose
        pose.pose.position.x = val.north; // Replace val.north with your north position value
        pose.pose.position.y = val.east; // Replace val.east with your east position value
        //pose.pose.position.z = (data.down - 37.81); // Replace with your z position calculation
        pose.pose.position.z = data_liorf.down; // Replace with your z position calculation
        

        // Set the orientation of the pose (assuming you have x, y, z, w orientation values)
        pose.pose.orientation.x = quat.x(); // Replace with your orientation x value
        pose.pose.orientation.y = quat.y(); // Replace with your orientation y value
        pose.pose.orientation.z = quat.z(); // Replace with your orientation z value
        pose.pose.orientation.w = quat.w(); // Replace with your orientation w value

        // Add the pose to the path
        path_msg.poses.push_back(pose);

        // Publish the path
        gps_path_pub.publish(path_msg);

        // Check RPY
        tf::Quaternion slam_quat(slam_rot_val.x, slam_rot_val.y, slam_rot_val.z, slam_rot_val.w);
        float slam_roll, slam_pitch, slam_yaw;
        quaternionToRPY(slam_roll, slam_pitch, slam_yaw, slam_quat);  */
