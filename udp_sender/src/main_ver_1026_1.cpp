...
int main(int argc, char **argv) {
    ...
    ros::Subscriber odom_sub = nh.subscribe("/liorf/mapping/odometry", 10, odomCallback);
    ...
    while (ros::ok()) {
        ...
        // init data
        if(count < 100){
            init_heading = data.heading;
            init_east  = data.east;
            init_north  = data.north;
        }
        init_heading = (-init_heading)*(3.141592/180.0);
        ...
        //vecxy(0) = msg->pose.pose.position.x;
        //vecxy(1) = -msg->pose.pose.position.y;
        ...
        // Value check
        std::cout << "udp heading    :: " << udp_data.heading*(180/3.141592) << std::endl;
